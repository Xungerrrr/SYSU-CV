...
def trainModel(model, modelDir):
	...
	# 读取图像
	with open("./ImageData/train-images.idx3-ubyte", 'rb') as imagesFile:
	...
	# 读取标签
	with open("./ImageData/train-labels.idx1-ubyte", 'rb') as labelsFile:
	...
	# 读取图像
	with open("./ImageData/t10k-images.idx3-ubyte", 'rb') as imagesFile:
		magicNumber, numberOfImages, rows, cols = struct.unpack('>IIII', imagesFile.read(16))
		testImages = np.fromfile(imagesFile, dtype=np.uint8).reshape(numberOfImages, 28 * 28)
		testImages = testImages.astype('int32')
	# 读取标签
	with open("./ImageData/t10k-labels.idx1-ubyte", 'rb') as labelsFile:
		magicNumber, numberOfItems = struct.unpack('>II', labelsFile.read(8))
...
writer = pd.ExcelWriter('Excel/predict.xlsx', engine='xlsxwriter')
